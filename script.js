/* ==================================================================
   THERA CONNECT â script.js v4.0
================================================================== */
...
function initSupabase(url, key) {
    ...
    console.log('ð Supabase initialisÃ©');
}

/* ==================================================================
   BARRE NAVIGATION MOBILE â hide/show au scroll
================================================================== */
...
async function loadAllData() {
    ...
    try {
        ...
        console.log('â DonnÃ©es chargÃ©es :', state.acces.length, 'accÃ¨s,', state.profils.length, 'profils');
    } catch (err) {
...
function applyPermissions(userRole, expiryDate = null) {
    ...
    if (role === 'super_admin') {
    ...
    } else if (role === 'rÃ©gulier' || role === 'regulier') {
        ...
        if (expiryDate) {
            ...
            notice.innerHTML = `â±ï¸ AccÃ¨s temporaire Â· Expire le <strong>${exp.toLocaleString('fr-FR')}</strong>${diff > 0 && diff < 72 ? ` Â· <span style="color:var(--red)">dans ${diff}h</span>` : diff <= 0 ? ' Â· <span style="color:var(--red)">EXPIRÃ</span>' : ''}`;
        }
    }
}
...
function _injectSuperAdminBadge() {
    ...
    badge.textContent = 'â SUPER ADMIN';
...
function _renderUserDashboard(profil) {
    ...
    if (!myAccess.length) {
        container.innerHTML = '<div class="empty-state"><p>Aucun accÃ¨s autorisÃ© pour votre profil.</p></div>';
        return;
    }
        ...
        card.innerHTML = `
            <div class="tile-header"><div class="tile-info">
                ...
                <span style="font-size:.78rem;color:${allowed ? 'var(--green)' : 'var(--text-muted)'}">
                    ${allowed ? 'â AccÃ¨s autorisÃ©' : 'â° Hors plage horaire'}
                </span>
            </div></div>
            <div class="tile-body"><div class="ip-badge"><i data-lucide="network"></i> ${acc.ip}</div></div>
            <div class="tile-actions">
                ${allowed
                    ? `<button class="btn-action-primary" onclick="openControlModal('${acc.id}','${acc.name}')"><i data-lucide="unplug"></i> ContrÃ´ler</button>`
...
function renderFavorites() {
    ...
    if (!favs.length) {
        container.innerHTML = '<div class="empty-state"><p>Aucun accÃ¨s en favori. Ajoutez des Ã©toiles â dans la page AccÃ¨s.</p></div>';
        return;
    }
        ...
        card.innerHTML = `
            <div class="main-info">
                ...
                <div>
                    ...
                    <div class="ip-badge"><i data-lucide="network"></i>${acc.ip || 'â'}</div>
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:8px">
                ...
                <div style="display:flex;gap:8px;padding:8px 0 4px;">
                    <button onclick="event.stopPropagation();sendCommand('${acc.id}','open')" style="flex:1;padding:8px;background:#22c55e;color:#fff;border:none;border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;">OUVRIR</button>
                    <button onclick="event.stopPropagation();sendCommand('${acc.id}','close')" style="flex:1;padding:8px;background:#ef4444;color:#fff;border:none;border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;">FERMER</button>
                </div>
            </div>`;
        container.appendChild(card);
    });
...
function openFavoriteModal(accId) {
    ...
    const modStatus = state.moduleStatus ? state.moduleStatus.find(m => m.access_id === acc.id) : null;
    const isOnline = modStatus ? modStatus.online : false;
    const isOpen = modStatus ? modStatus.door_open : false;

    const existing = document.getElementById('fav-slide-panel');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'fav-slide-panel';
    ...
    panel.style.cssText = 'width:100%;background:#1e1e2e;border-radius:20px 20px 0 0;padding:24px;box-sizing:border-box;transform:translateY(100%);transition:transform 0.3s ease;';

    const statusColor = isOnline ? (isOpen ? '#ef4444' : '#22c55e') : '#6b7280';
    const statusText = isOnline ? (isOpen ? 'OUVERT' : 'FERM\u00C9') : 'HORS LIGNE';
    const openBg = isOpen ? '#374151' : '#22c55e';
    const closeBg = isOpen ? '#ef4444' : '#374151';

    panel.innerHTML = [
        '<div style="width:40px;height:4px;background:#444;border-radius:2px;margin:0 auto 20px;"></div>',
        '<h3 style="margin:0 0 8px;color:#fff;font-size:18px;">' + (acc.nom || acc.name || 'Acc\u00E8s') + '</h3>',
        '<p style="margin:0 0 20px;color:' + statusColor + ';font-weight:600;font-size:14px;">' + statusText + '</p>',
        '<div style="display:flex;gap:12px;margin-bottom:16px;">',
        '<button id="fav-btn-open" style="flex:1;padding:14px;background:' + openBg + ';color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;">OUVRIR</button>',
        '<button id="fav-btn-close" style="flex:1;padding:14px;background:' + closeBg + ';color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;">FERMER</button>',
        '</div>',
        '<button id="fav-btn-cancel" style="width:100%;padding:12px;background:#374151;color:#9ca3af;border:none;border-radius:12px;font-size:14px;cursor:pointer;">Annuler</button>'
    ].join('');
    ...
    document.getElementById('fav-btn-cancel').addEventListener('click', () => overlay.remove());
    document.getElementById('fav-btn-open').addEventListener('click', () => { sendCommand(accId, 'open'); overlay.remove(); });
    document.getElementById('fav-btn-close').addEventListener('click', () => { sendCommand(accId, 'close'); overlay.remove(); });
}
...
function confirmFavoriteCommand(accId, action) {
    ...
    showToast(`â Commande ${label} envoyÃ©e`);
...
/* ==================================================================
   COMMANDES ESP32 â Modale + Slider de confirmation
================================================================== */
function openControlModal(accId, accName) {
    ...
    // RÃ©initialiser l'Ã©tat de la modale
...
function _resetSlider() {
    ...
    if (label) { label.textContent = 'Glissez pour ouvrir â'; label.style.opacity = '1'; label.style.color = ''; }
}
...
function _initSlider() {
    ...
    function complete() {
        ...
        if (label) { label.textContent = 'ConfirmÃ© â'; label.style.opacity = '1'; label.style.color = 'white'; }
...
async function _executeCommand(action) {
    ...
    if (!httpOk) {
        showToast('WiFi indisponible â tentative Bluetooth...', 'info');
        const btOk = await _sendBluetoothCommand(acc, action);
        if (!btOk) {
            navigator.vibrate?.([200, 100, 200]);
            showToast(`Commande ${action} Ã©chouÃ©e`, 'error');
            return;
        }
    }
    ...
    const profil = typeof getCurrentProfil === 'function' ? getCurrentProfil() : null;
    addHistoryEntry(profil?.name || 'SystÃ¨me', acc.name, action, httpOk ? 'WiFi' : 'Bluetooth');
    navigator.vibrate?.(60);
    showToast(`â ${acc.name} â ${action}`, 'success');
    renderAccueil();
}

async function confirmCommand(action) {
    // AppelÃ© uniquement pour FERMER (OUVRIR passe par le slider)
    closeModal('modal-control');
...
function openPlanningEditor(accId) {
    ...
    if (titleEl) titleEl.textContent = `Planning â ${acc.name}`;
    ...
    return (planningSlots[di] || []).map((slot, i) => {
        ...
        return `
            <div class="cal-slot" draggable="true" style="top:${top}px;height:${height}px"
                ...
                <button class="cal-slot-del" onclick="event.stopPropagation();_calDeleteSlot(${di},${i})">Ã</button>
...
async function savePlanning() {
    ...
    try {
        ...
        showToast(`â Planning enregistrÃ© pour ${acc.name}`);
    } catch (err) { showToast('Erreur : ' + err.message, 'error'); }
}

function clearPlanning() {
    if (confirm('Effacer tous les crÃ©neaux ?')) { planningSlots = {}; _renderCalendar(); }
}
...
function renderAccesList() {
    ...
    if (!state.acces.length) {
        container.innerHTML = '<div class="empty-state"><p>Aucun point d\'accÃ¨s configurÃ©.</p></div>';
        return;
    }
    state.acces.forEach(acc => {
        const onlineDot = acc._online === true  ? '<span style="color:var(--green);font-size:.7rem">â</span>'
                        : acc._online === false ? '<span style="color:var(--red);font-size:.7rem">â</span>' : '';
        const card = document.createElement('div');
        card.className = 'card item-card animate-in';
        ...
        card.innerHTML = `
            ...
            <div style="display:flex;gap:8px;align-items:center;">
                <button class="btn-small" onclick="event.stopPropagation();toggleFavorite('${acc.id}')"
                    ...
                    style="color:${acc.is_favorite ? 'var(--orange)' : 'var(--text-muted)'};">â</button>
...
async function toggleFavorite(id) {
    ...
    showToast(newVal ? 'â AjoutÃ© aux favoris' : 'â RetirÃ© des favoris');
    renderAccesList();
}
...
function createNewAcces() {
    ...
    if (!supabaseClient)  { showToast('Supabase non connectÃ©', 'error'); return; }

    const entry = { name, ip, relay_id: 1, is_favorite: false, is_deleted: false, planning: { slots: {} } };
    showToast('CrÃ©ation en cours...', 'info');
...
function deleteCurrentAcces() {
    if (confirm('Envoyer Ã  la corbeille ?')) _deleteToTrash('acces', currentEditingId);
}
...
function renderESP32StatusPanel(acc) {
    ...
    panel.innerHTML = `
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px;">
            <div class="esp32-badge" id="esp-badge-wifi">ð¶ WiFi</div>
            <div class="esp32-badge" id="esp-badge-ip">ð IP</div>
            <div class="esp32-badge" id="esp-badge-relay">â¡ Relais</div>
        </div>
...
async function pingCurrentAcces() {
    ...
    _setESPBadge('wifi',  'pending', 'ð¶ â¦');
    _setESPBadge('ip',    'pending', 'ð â¦');
    _setESPBadge('relay', 'pending', 'â¡ â¦');
    ...
    try {
        ...
        _setESPBadge('wifi',  'ok', 'ð¶ WiFi â');
        _setESPBadge('ip',    'ok', `ð ${acc.ip} â`);
        _setESPBadge('relay', 'ok', `â¡ ${data?.relays?.[0]?.state || 'ok'}`);
        if (infoLine) infoLine.textContent = `RSSI : ${data?.rssi ?? 'â'} dBm Â· RÃ©ponse OK`;

        const idx = state.acces.findIndex(a => a.id === currentEditingId);
        if (idx !== -1) state.acces[idx]._online = true;
    } catch {
        _setESPBadge('wifi',  'err', 'ð¶ â');
        _setESPBadge('ip',    'err', 'ð â');
        _setESPBadge('relay', 'err', 'â¡ â');
...
async function scanBLE() {
    if (!navigator.bluetooth) { showToast('Bluetooth non supportÃ© sur ce navigateur', 'error'); return; }
    const resultsEl = document.getElementById('ble-scan-results');
    if (resultsEl) { resultsEl.style.display = 'block'; resultsEl.innerHTML = '<em style="font-size:.82rem;color:var(--text-muted)">Scan en coursâ¦</em>'; }
    try {
        ...
        if (resultsEl) resultsEl.innerHTML = `
            <div class="esp32-ble-device">
                <span>ðµ <strong>${device.name || 'Appareil sans nom'}</strong></span>
                <button class="btn-small" onclick="showToast('Appareil BLE sÃ©lectionnÃ© : ${device.name}')">SÃ©lectionner</button>
            </div>`;
    } catch {
        if (resultsEl) resultsEl.innerHTML = '<em style="font-size:.82rem;color:var(--text-muted)">Scan annulÃ©.</em>';
    }
}
...
function renderProfilsList() {
    ...
    container.innerHTML = '';
    container.className = 'responsive-grid';
    const colors = { 'super_admin':'#f0883e', 'Administrateur':'var(--blue)', 'RÃ©gulier':'var(--green)', 'Visiteur':'var(--text-muted)' };
    ...
    visible.forEach(prof => {
        const role  = prof.type || 'RÃ©gulier';
            ...
            <div class="main-info">
                ...
                <div>
                    ...
                    <div style="display:flex;gap:6px;align-items:center;margin-top:3px;">
                        ...
                        ${prof.expires_at ? `<span style="font-size:.75rem;color:var(--orange)">Â· ${new Date(prof.expires_at).toLocaleDateString('fr-FR')}</span>` : ''}
                    </div>
                </div>
            </div>
            <div style="display:flex;gap:8px;align-items:center;">
...
function createNewProfil() {
    ...
    const type   = document.getElementById('new-prof-type')?.value         || 'RÃ©gulier';
...
function openProfilEdit(id) {
    ...
    document.getElementById('edit-prof-type').value   = prof.type   || 'RÃ©gulier';
    ...
    // DÃ©sactiver Supprimer pour Super Admin
...
function _renderProfilAccessRights(prof) {
    ...
    if (!state.acces.length) {
        container.innerHTML = '<p style="color:var(--text-muted);font-size:.85rem">Aucun accÃ¨s configurÃ©.</p>';
        return;
    }
    container.innerHTML = state.acces.map(acc => `
        <label style="display:flex;align-items:center;gap:10px;padding:8px 0;cursor:pointer;border-bottom:1px solid var(--border)">
            ...
            <span class="ip-badge">${acc.ip || 'â'}</span>
        </label>`).join('');
}
...
async function saveProfilModifications() {
    ...
    if (code && state.profils.some(p => p.code === code && p.id !== currentEditingProfilId)) {
        showToast('â ï¸ Ce code est dÃ©jÃ  utilisÃ©', 'error');
        return;
    }
    const VALID_TYPES = ['super_admin', 'Administrateur', 'RÃ©gulier', 'Visiteur'];
    ...
    const safeType = VALID_TYPES.includes(rawType) ? rawType : (currentProfil?.type || 'RÃ©gulier');
    if (!safeType) {
        showToast('â ï¸ Type de profil invalide', 'error');
        return;
    }
...
function deleteCurrentProfil() {
    ...
    if (_prof && _prof.type === 'super_admin') {
        showToast('â Le super administrateur ne peut pas Ãªtre supprimÃ©', 'error');
        return;
    }
    
    if (confirm('Envoyer Ã  la corbeille ?')) _deleteToTrash('profil', currentEditingProfilId);
}
...
function openInvitationModal(email, name, role, phone) {
    ...
    const msg    = `Bonjour ${name}, voici votre lien d'accÃ¨s Thera Connect : ${url}`;
    const msgEnc = encodeURIComponent(msg);
    const tel    = (phone || '').replace(/\s/g, '');

    modal.innerHTML = `
        <div class="modal-content" style="max-width:420px;">
            <h3 style="margin-bottom:6px;">ð¨ Inviter ${name}</h3>
            <p style="color:var(--text-sub);font-size:.85rem;margin-bottom:16px;">RÃ´le : <strong>${role}</strong></p>
            ...
            <div style="display:flex;flex-direction:column;gap:10px;">
                ${email ? `<a href="mailto:${email}?subject=Votre accÃ¨s Thera Connect&body=${msgEnc}" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:white;color:var(--text-main);text-decoration:none;font-weight:600;font-size:.875rem;"><span style="font-size:1.3rem">ð§</span><div><div>Email</div><div style="font-size:.73rem;color:var(--text-sub);font-weight:400">${email}</div></div></a>` : '<div style="padding:10px;border-radius:var(--radius);background:var(--card-inner);color:var(--text-muted);font-size:.83rem;text-align:center;">Email non renseignÃ©</div>'}
                ${tel ? `
                <a href="sms:${tel}?body=${msgEnc}" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:white;color:var(--text-main);text-decoration:none;font-weight:600;font-size:.875rem;"><span style="font-size:1.3rem">ð¬</span><div><div>SMS</div><div style="font-size:.73rem;color:var(--text-sub);font-weight:400">${phone}</div></div></a>
                <a href="https://wa.me/${tel.replace('+','')}?text=${msgEnc}" target="_blank" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:white;color:var(--text-main);text-decoration:none;font-weight:600;font-size:.875rem;"><span style="font-size:1.3rem">ð¢</span><div><div>WhatsApp</div><div style="font-size:.73rem;color:var(--text-sub);font-weight:400">${phone}</div></div></a>` : '<div style="padding:10px;border-radius:var(--radius);background:var(--card-inner);color:var(--text-muted);font-size:.83rem;text-align:center;">TÃ©lÃ©phone non renseignÃ©</div>'}
            </div>
...
function renderAlertRules() {
    ...
    if (!container) return;
    const typeLabels = { stay_open:'ðª Ouvert trop longtemps', wrong_hours:'ð Hors-horaires', hardware_cell:'â DÃ©faut capteur' };
    if (!state.alertRules.length) {
        container.innerHTML = '<p style="color:var(--text-muted);font-size:.85rem;text-align:center;padding:16px">Aucune rÃ¨gle configurÃ©e.</p>';
        return;
    }
    container.innerHTML = state.alertRules.map((r, i) => {
        ...
        return `<div class="alert-item" style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                ...
                <small>${acc ? acc.name : 'AccÃ¨s inconnu'}${r.duration ? ` Â· ${r.duration} min` : ''}</small>
            </div>
            <button class="btn-small" style="color:var(--red)" onclick="deleteAlertRule(${i})">â</button>
        </div>`;
    }).join('');
}
...
async function saveNewAlertRule() {
    ...
    try {
        ...
        showToast('â RÃ¨gle enregistrÃ©e');
        showAlertSubPage('setup');
    } catch (err) { showToast('Erreur : ' + err.message, 'error'); }
}

async function deleteAlertRule(ruleId) {
    if (!confirm('Supprimer cette rÃ¨gle d\'alerte ?')) return;
    try {
        if (supabaseClient) {
...
function renderAlertLogs() {
    ...
    const typeLabels = {
        stay_open: 'ðª Portail ouvert trop longtemps',
        wrong_hours: 'ð Ouverture hors-horaires',
        hardware_cell: 'â¡ DÃ©faut capteur'
    };

    if (!state.alertRules || !state.alertRules.length) {
        container.innerHTML += '<p style="color:var(--text-muted);font-size:.9rem;margin-bottom:8px">Aucune rÃ¨gle configurÃ©e.</p>';
    } else {
        const rulesTitle = document.createElement('h4');
        rulesTitle.textContent = 'RÃ¨gles actives';
        ...
        state.alertRules.forEach(rule => {
            ...
            el.innerHTML = `<div>
                ...
                <div style="font-size:.8rem;color:var(--text-muted)">${acc ? acc.name : 'AccÃ¨s inconnu'} â ${rule.duration || 'â'} min</div>
            </div>
            <button class="btn-danger" style="padding:4px 10px;font-size:.8rem" onclick="deleteAlertRule('${rule.id}')">ðï¸</button>`;
    ...
    if (state.alertLogs && state.alertLogs.length > 0) {
        ...
        logsTitle.textContent = 'Journal des Ã©vÃ©nements';
        ...
        if (!recentLogs.length) {
            container.innerHTML += '<p style="color:var(--text-muted);font-size:.9rem">Aucun Ã©vÃ©nement rÃ©cent.</p>';
        } else {
            recentLogs.slice(-50).reverse().forEach(log => {
                ...
                el.textContent = d.toLocaleString('fr-FR') + ' â ' + (log.message || log.type || 'Alerte');
                container.appendChild(el);
            });
        }
    }
...
function setKeyFilter(f) {
    ...
    document.querySelectorAll('.chip').forEach(btn => {
        const map = { all:'Tout', code:'Codes', badge:'Badges', remote:'TÃ©lÃ©commandes' };
...
function renderKeysList() {
    ...
    state.profils.forEach(prof => {
        [{ type:'code', val:prof.code, icon:'ð¢' },
         { type:'badge', val:prof.badge, icon:'ðªª' },
         { type:'remote', val:prof.remote, icon:'ð¡' }].forEach(k => {
            if (!k.val) return;
    ...
    if (!tbody.innerHTML) {
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:24px;color:var(--text-muted)">Aucune clÃ© trouvÃ©e.</td></tr>';
    }
}
...
function renderHistory() {
    ...
    state.historique.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td>${item.user   || 'SystÃ¨me'}</td>
            <td>${item.access || 'GÃ©nÃ©ral'}</td>
...
function exportHistory() {
    if (!state.historique.length) { showToast('Rien Ã  exporter', 'error'); return; }
    const csv  = 'Date;Heure;Utilisateur;AccÃ¨s;Action;Source\n' +
...
async function _deleteToTrash(type, id) {
    ...
    try {
        ...
        list.splice(idx, 1);
        showToast('ðï¸ DÃ©placÃ© dans la corbeille');
        showPage(type === 'acces' ? 'page-acces' : 'page-profils');
    } catch (err) { showToast('Erreur : ' + err.message, 'error'); }
}

/* ==================================================================
   TEMPS RÃEL â Ping 30s + Supabase Realtime + Badge alertes
================================================================== */
...
function startRealtimeFeatures() {
    ...
    supabaseClient.channel('realtime-logs')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'logs' }, payload => {
            ...
            state.alertLogs.unshift({ timestamp: l.timestamp, text: `${l.action} â ${l.details}` });
            _updateAlertBadge();
...
function renderSystemModules() {
    ...
    container.innerHTML = '';
    if (!state.acces.length) {
        container.innerHTML = '<div class="empty-state"><p>Aucun module configurÃ©.</p></div>';
        return;
    }
    state.acces.forEach(acc => {
        ...
        div.innerHTML = `
            <div>
                ...
                <small style="font-family:monospace">IP: ${acc.ip} Â· Relay: ${acc.relay_id||1}</small>
            </div>
...
async function pingModule(ip) {
    showToast(`ð¡ Ping ${ip}...`, 'info');
    try {
        const ctrl = new AbortController();
        setTimeout(() => ctrl.abort(), 3000);
        await fetch(`http://${ip}/`, { signal: ctrl.signal, mode: 'no-cors' });
        showToast(`â ${ip} rÃ©pond`, 'success');
    } catch { showToast(`â ${ip} injoignable`, 'error'); }
}

function rebootModule(name, ip) {
    if (!confirm(`RedÃ©marrer "${name}" ?`)) return;
    fetch(`http://${ip}/reboot`, { mode: 'no-cors' }).then(r=>r).catch(() => {});
    showToast(`ð Reboot envoyÃ© Ã  ${name}`);
}
...
function saveAppSettings() {
    ...
    showToast('â¨ Apparence mise Ã  jour');
}
...
function saveSupabaseConfig() {
    ...
    loadAllData();
    startCloudHealthPolling();
    showToast('ð¾ Configuration enregistrÃ©e');
}
...
async function checkCloudHealth(silent = false) {
    ...
    if (badge) { badge.textContent = 'â³ VÃ©rificationâ¦'; badge.style.color = 'var(--text-muted)'; }
    if (!supabaseClient) {
        if (badge) { badge.textContent = 'â ÃCHEC â non configurÃ©'; badge.style.color = 'var(--red)'; }
        if (!silent) showToast('Supabase non initialisÃ©', 'error');
        return;
    }
    ...
    if (badge) {
        badge.textContent = ok ? `â Connexion Cloud OK Â· ${ts}` : `â ÃCHEC Â· ${ts}`;
        badge.style.color = ok ? 'var(--green)' : 'var(--red)';
    }
    if (!silent) showToast(ok ? 'â Connexion rÃ©ussie !' : `â ${error.message}`, ok ? 'success' : 'error');
}
    ...
    supabaseClient.from('settings').select('id').limit(1)
        ...
        .then(({ error }) => {
            ...
            badge.innerHTML = error 
                ...
                : '<span class="dot-online"></span> Cloud ConnectÃ©';
        });
}
...
window.onload = () => {
    console.log('ð Thera Connect v4.0');
    if (window.lucide) lucide.createIcons();
};
